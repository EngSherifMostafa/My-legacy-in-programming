...
//split original queue into even and odd queues
template <class T>
void QueueLinked<T>::split(QueueLinked<T> &evenQ,QueueLinked<T> &oddQ) const
{
	if(isEmpty()) return;
	nodePtr temp=front;
	...
	{
		if(temp->data%2==0)
			evenQ.enQueue(temp->data);
		else
			oddQ.enQueue(temp->data);
