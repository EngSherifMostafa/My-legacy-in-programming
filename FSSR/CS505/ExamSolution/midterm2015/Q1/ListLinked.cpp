...
//insert using key without using any method
template <class K,class T>
void List<K,T>::insertByKey(const K sentKey,const T sentData)
{
	nodePtr newNode=new Node(sentKey,sentData);
	...
	cursor=head;
	while(cursor->key<sentKey)
	{
		prev=cursor;
		cursor=cursor->next;
		//if there are no key less than sent key cursor reach to NULL
		if(!cursor) break;
	}
...
//remove minimum key (minimum key in ordered linked list is at head)
template <class K,class T>
void List<K,T>::remMin(K &retKey,T &retData)
{
	if(!head) return;
	nodePtr tempNode=head;
	retKey=head->key;
