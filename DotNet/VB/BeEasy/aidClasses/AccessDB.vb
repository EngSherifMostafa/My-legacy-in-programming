...
Imports System.Data.OleDb 'access db
Imports System.Text.RegularExpressions

Public Class AccessDB

    Private conAccess As New OleDbConnection()
    Private cmdAccess As New OleDbCommand()

    'connection .accdb file
    Function connectDB(ByRef dbPath As String, Optional ByRef msg As String = Nothing) As Boolean
        Try
            conAccess = New OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" & dbPath)
            conAccess.Open()
            conAccess.Close()
            Return True
    ...
    Function runDDL(ByRef ddlStr As String, Optional ByRef msg As String = Nothing) As Boolean
        ...
        cmdAccess = New OleDbCommand(ddlStr, conAccess)
        Try
        ...
        Catch ex As Exception
            ...
            Return 0
        Finally
            If (conAccess.State() = ConnectionState.Open) Then conAccess.Close()
        End Try
    End Function
    ...
    Function runDML(ByRef dmlStr As String, Optional ByRef msg As String = Nothing) As DataTable
        ...
        If (conAccess.State() = ConnectionState.Closed) Then conAccess.Open()
        Try
            cmdAccess = New OleDbCommand(dmlStr, conAccess)
            dataTbl.Load(cmdAccess.ExecuteReader())
        Catch ex As Exception
